...
if [[ -z "$SSH_KEY" || -z "$EC2_USER" || -z "$EC2_IP" || -z "$REMOTE_PATH" || -z "$REPO_URL" || -z "$APP_NAME" || -z "$NODE_ENV" || -z "$GIT_BRANCH" || -z "$NATS_SERVERS" || -z "$STRIPE_SECRET" || -z "$ENDPOINT_SECRET" ]]; then
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$EC2_USER@$EC2_IP" "
  ...
  echo \"ENDPOINT_SECRET=$ENDPOINT_SECRET\" >> .env
  echo \"STRIPE_SUCCESS_URL =http://$EC2_IP:8080/payments/success\" >> .env
  echo \"STRIPE_CANCEL_URL =http://$EC2_IP:8080/payments/cancelled\" >> .env
