pipeline {
    ...
    environment {
        ...
        K8S_REMOTE_PATH = '/home/ubuntu/nest-microservices/k8s/store-ms/templates/payment-ms/'
        IMAGE_NAME = 'fernandoflores07081/payment-ms-prod'
    }
        ...
        stage('Deploy to GKE - Production') {
            ...
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/payment-ms payment-ms-prod=${IMAGE_NAME}:${env.DOCKER_TAG} --namespace=default"

                    // Verificar que el despliegue se complete exitosamente
                    sh "kubectl rollout status  deployment/payment-ms --namespace=default"
                }
            }
        }
